...
angular.module('jewelryApp').controller('ItemDialogController',
    ['$scope', '$stateParams', '$modalInstance', 'entity', 'Item', 'ItemInformation', 'Category', 'Price', 'Sizes', 'Color', 'Image', 'PurchaseOrderDetails',
        function($scope, $stateParams, $modalInstance, entity, Item, ItemInformation, Category, Price, Sizes, Color, Image, PurchaseOrderDetails) {

        $scope.item = entity;
        $scope.iteminformations = ItemInformation.query();
        $scope.categorys = Category.query();
        $scope.prices = Price.query();
        $scope.sizess = Sizes.query();
